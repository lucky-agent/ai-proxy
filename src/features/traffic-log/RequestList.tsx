...
import { ArrowDownIcon, ArrowUpIcon, CopyIcon, CheckIcon, RefreshCwIcon } from 'lucide-react'
...
// ---------------------------------------------------------------------------
const Grip = ({
  className, onPointerDown, onPointerMove, onPointerUp
}: {
  className?: string
  ...
  <span className={cn('shrink-0 cursor-col-resize p-[2px] -mr-[2px] relative z-10', className)}
...
// RequestList
// ---------------------------------------------------------------------------
export default function RequestList({ entries, selectedId, onSelectEntry, sortOrder, onSortOrderChange, onResendRequest }: Props) {
  const { t } = useTranslation()
  ...
  return (
    ...
    >
      ...
      <div className="grid shrink-0 z-10 bg-muted/30 text-[11px] font-bold text-muted-foreground uppercase tracking-wide border-b border-border" style={gridColsStyle}>
        ...
        <div className="px-1.5 py-1.5 text-left">
          <div className="flex items-center gap-1">
            <Grip className={draggingCol === 'id' ? 'text-primary' : 'text-muted-foreground/25 hover:text-muted-foreground/70'}
        ...
        <div className="px-1.5 py-1.5 text-left">
          <div className="flex items-center gap-1">
            <Grip className={draggingCol === 'url' ? 'text-primary' : 'text-muted-foreground/25 hover:text-muted-foreground/70'}
              onPointerMove={onPointerMove} onPointerUp={onPointerUp}
        ...
        <div className="px-1.5 py-1.5 text-left">
          <div className="flex items-center gap-1">
            <Grip className={draggingCol === 'method' ? 'text-primary' : 'text-muted-foreground/25 hover:text-muted-foreground/70'}
              onPointerMove={onPointerMove} onPointerUp={onPointerUp}
        ...
        <div className="px-1.5 py-1.5 text-left">
          <div className="flex items-center gap-1">
            <Grip className={draggingCol === 'status' ? 'text-primary' : 'text-muted-foreground/25 hover:text-muted-foreground/70'}
              onPointerMove={onPointerMove} onPointerUp={onPointerUp}
        ...
        <div className="px-1.5 py-1.5 text-right">
          <div className="flex items-center justify-end gap-1">
            <Grip className={draggingCol === 'duration' ? 'text-primary' : 'text-muted-foreground/25 hover:text-muted-foreground/70'}
              onPointerMove={onPointerMove} onPointerUp={onPointerUp}
              onPointerDown={e => onPointerDown('duration', e)} />
            <button type="button" className="cursor-pointer hover:text-foreground transition-colors whitespace-nowrap"
              onClick={() => onSortOrderChange(sortOrder === 'desc' ? 'asc' : 'desc')}>
              {sortOrder === 'desc' ? (<ArrowDownIcon className="inline size-3 mr-0.5" />) : (<ArrowUpIcon className="inline size-3 mr-0.5" />)}
              {t('requestList.time')}
            </button>
          </div>
        </div>
        <div className="px-1.5 py-1.5 text-left">
          <div className="flex items-center gap-1">
            <Grip className={draggingCol === 'time' ? 'text-primary' : 'text-muted-foreground/25 hover:text-muted-foreground/70'}
              onPointerMove={onPointerMove} onPointerUp={onPointerUp}
        ...
        <VList key={sortOrder} style={{ flex: 1, minHeight: 0 }}>
          {entries.map((entry) => (
            ...
            >
              ...
              <div className="px-1 py-2 text-foreground/80 min-w-0" title={entry.uri}>
                <span className="block truncate">{shortenUri(entry.uri)}</span>
              </div>
              <div className="px-1 py-2">
                <span className="badge-method" style={badgeStyle(`--badge-${entry.method.toLowerCase()}`)}>{entry.method}</span>
              </div>
              <div className="px-1 py-2">
                <span className="badge-status" style={badgeStyle(`--badge-${statusCategory(entry.status)}`)}>{entry.status ?? t('requestList.pending')}</span>
              </div>
              ...
              <div className="px-1 py-2 text-center">
                {entry.edited ? (<span className="text-[10px] text-amber-400 font-medium">{t('requestList.edited')}</span>) : null}
              </div>
            </div>
          ))}
        </VList>
      ...
      {/* 右键菜单 Portal */}
      {ctxMenu && (
        <ContextMenu state={ctxMenu} onClose={() => setCtxMenu(null)} onResend={onResendRequest} />
      )}
    </div>
  )
}
